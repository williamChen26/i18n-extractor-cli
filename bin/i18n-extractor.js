...
const ignoreTypes = ['TSLiteralType', 'ImportDeclaration'];

const ignoreFns = ['t', 'i18n', 'translate'];

const attributesToExtract = ['title', 'placeholder', 'alt']; // 需要处理的属性列表
  ...
  for (let index = 0; index < files.length; index++) {
      ...
      VariableDeclarator(path) {
        if (path.node.init) {
          if (path.node.init.type === 'StringLiteral') {
            ...
            variables[path.node.id.name] = path.node.init.elements
        ...
        if (path.node.right) {
          if (path.node.right.type === 'StringLiteral') {
            if (path.node.left.type === 'Identifier') {
              variables[path.node.left.name] = path.node.right.value;
            }
          } else if (path.node.right.type === 'ArrayExpression') {
            if (path.node.left.type === 'Identifier') {
              variables[path.node.left.name] = path.node.right.elements
                .filter((el) => el.type === 'StringLiteral')
                .map((el) => el.value);
            }
          }
    ...
    traverse.default(ast, {
      ...
      JSXText(path) {
        ...
        //   debugger;
        // }
        if (text && !isInsideI18nCall(path) && !ignoreTypes.includes(path.parent.type)) {
      ...
      StringLiteral(path) {
        ...
        if (path.parentPath.isJSXExpressionContainer() && !path.findParent((p) => p.isJSXAttribute())) {
          const text = path.node.value.trim();
      ...
      JSXExpressionContainer(path) {
        ...
        if (expression.type === 'Identifier' && !path.findParent((p) => p.isJSXAttribute())) {
          const variableName = expression.name;
          const text = variables[variableName] || getPropValue(variableName, path);
          if (Array.isArray(text)) {
            text.forEach((item) => {
              addTranslation(file, item.trim());
            });
          } else if (text && typeof text === 'string' && !isInsideI18nCall(path)) {
            addTranslation(file, text.trim());
            // translations[text.trim()] = text.trim();
          }
        } else if (expression.type === 'MemberExpression' && expression.object.type === 'Identifier') {
        ...
        if (callee.type === 'MemberExpression' && callee.property.name === 'map') {
          ...
          if (variables[arrayName] && Array.isArray(variables[arrayName])) {
            variables[arrayName].forEach((item) => {
              if (item && typeof item === 'string' && !isInsideI18nCall(path)) {
                addTranslation(file, item.trim());
                // translations[item.trim()] = item.trim();
              }
            });
          }
        }
      ...
      JSXAttribute(path) {
        ...
        if (attributesToExtract.includes(attributeName) && path.node.value.type === 'StringLiteral') {
...
function isInsideI18nCall(path) {
  ...
  while (path) {
    ...
    //   return true;
    // }
    if (path.type === 'CallExpression' && path.callee?.type === 'Identifier' && ignoreFns.includes(path.callee?.name)) {
      return true;
    }
...
function getPropValue(variableName, path) {
  // 假设父组件传递的prop是字符串字面量
  const propPath = path.findParent((p) => p.isJSXOpeningElement());
  if (propPath) {
    const prop = propPath.node.attributes.find((attr) => attr.name && attr.name.name === variableName);
